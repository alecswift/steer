...
| Map layers | Hillshade, contours and trails are **always on**. There are no layer toggles in the MVP; they're future work. Only the **route selected in the sidebar** is drawn. |
...
- [ ] **0.2 Pull map config and styles out of `App.tsx`**
...
- [ ] **0.3 Split the map into components**
...
- [ ] **0.5 Docker Compose with PostGIS**
...
- [ ] **0.6 Root README for developers**
...
**Milestone 0**: The frontend runs from `frontend/` as modular components, the database runs in Docker, and the test runner works.
...
- [ ] **1.2 Clickable peak layer**
  ...
  *Verify*: every peak in the 0.2 table shows as a labelled peak, and hovering one shows a pointer.
...
- [ ] **1.3 Sidebar shell and selection**
  ...
  - Clicking a peak selects it. Clicking empty map clears the peak only.
...
- [ ] **1.7 Curated exact links**
  - Add `src/data/peakLinks.ts`: exact SummitPost and WTA pages for the peaks in the 0.2 table.
...
| Requirement | Chunks |
|---|---|
| FR-001 Map layers; only the selected route drawn | 0.3, 4.3 |
